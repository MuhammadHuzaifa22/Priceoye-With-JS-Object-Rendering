...
function renderItems (){
    ...
    if(cartItems != null && cartItems.length > 0){
        for(let i = 0; i < cartItems.length; i++){
            ...
            div.innerHTML += `
            <div class="border border-dark rounded m-5 p-3">
            <h1>Model: ${cartItems[i].brand +' '+ cartItems[i].model}</h1>
            <h1>Quantity:
            <button class="btn btn-primary" onclick="addQuantity(${i})"><i class="fa-solid fa-cart-plus"></i></button>
            ${cartItems[i].quantity}
            <button class="btn btn-primary" onclick="lessQuantity(${i})"><i class="fa-solid fa-minus"></i></button>
            </h1>
        ...
        totalAmount.innerHTML = `Total Amount :` + " " + 0.00;
    }
}
